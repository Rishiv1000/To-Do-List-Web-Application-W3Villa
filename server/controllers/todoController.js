...
exports.updateTodo = async (req, res) => {
  try {
    ...
    // Find the current todo from the database
    const existingTodo = await Todo.findById(id);
    
    // If no changes were made (i.e., same values as the existing todo), return the current todo
    if (
      existingTodo.task === task &&
      existingTodo.completed === completed &&
      existingTodo.completed_time === completed_time
    ) {
      return res.json(existingTodo); // No changes, return the current todo as is
    }

    // If changes were made, update the todo
    ...
    res.json(updatedTodo); // Send updated todo as json
  } catch (error) {
